...
def make_request_stats(
    ...
    # Extract diffusion timings if present on outputs.metrics
    vae_time_ms = None
    dit_time_ms = None
    denoise_time_ms = None
    for _ro in req_output:
        metrics = getattr(_ro, "metrics", None)
        if isinstance(metrics, dict):
            if vae_time_ms is None:
                vae_time_ms = metrics.get("vae_time_ms", metrics.get("vae_time"))
            if dit_time_ms is None:
                dit_time_ms = metrics.get("dit_time_ms", metrics.get("dit_time"))
            if denoise_time_ms is None:
                denoise_time_ms = metrics.get("denoise_time_ms", metrics.get("denoise_time"))
        if vae_time_ms is not None and dit_time_ms is not None and denoise_time_ms is not None:
            break
